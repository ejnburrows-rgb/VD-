...
export async function analyzeDecimas(transcript: string, singerName?: string): Promise<AnalysisResult> {
  if (!GEMINI_API_KEY) {
...
[Repetir para cada poeta]`
    ...
    // Propagate specific errors with context
    ...
    if (errorMessage.includes('safety') || errorMessage.includes('blocked')) {
      throw new Error('Content was blocked by safety filters. Please try a different transcript.')
    }
