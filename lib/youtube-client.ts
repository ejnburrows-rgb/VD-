...
if (!YOUTUBE_API_KEY) {
  console.error('⚠️ YOUTUBE_API_KEY no está configurada en .env');
}
...
export async function getVideoInfo(videoId: string): Promise<YouTubeVideoInfo> {
  if (!YOUTUBE_API_KEY || YOUTUBE_API_KEY === 'YOUR_YOUTUBE_API_KEY_HERE') {
    throw new Error(
      'YouTube API Key no configurada. Por favor, agrega tu clave en el archivo .env como YOUTUBE_API_KEY=tu_clave_aqui'
    );
  }
  ...
  try {
    ...
    // Propagar el error original si no es uno conocido
    throw error;
  }
}
